...
namespace WalletWasabi.Tests
{
	public class CommandInterpreterTests
	{
		...
		{
			var runGui = await CommandInterpreter.ExecuteCommandsAsync(new string[0]);
			Assert.True(runGui);
		}
		...
		public async Task InvokedWithHelpAsync()
		{
			...
			var runGui = await CommandInterpreter.ExecuteCommandsAsync(new[]{ "help" });
			output.Flush();
			...
			ms.Flush();
			CommandInterpreter.Configure(CreateMixCommand(), CreateFindPasswordCommand(), output, output);
			runGui = await CommandInterpreter.ExecuteCommandsAsync(new[]{ "--help" });
			output.Flush();
		...
		public async Task InvokedWithDataDirAndMixAsync()
		{
			...
			mixCommand.Run = (s)=> invoked = true;
			CommandInterpreter.Configure(mixCommand, CreateFindPasswordCommand(), output, output);
			var runGui = await CommandInterpreter.ExecuteCommandsAsync(new[]{ "mix", "--datadir", "expected/passed/datadir" });
			output.Flush();
			Assert.Equal("expected/passed/datadir", WalletWasabi.Gui.Global.DataDir);
			Assert.True(invoked);
			Assert.False(runGui);
