	...
	public class RegTests : IClassFixture<SharedFixture>
	{
		...
		public async Task WalletTestsAsync()
		{
			...
			try
			{
				...
				Assert.Single(wallet.Coins.Items);
				var firstCoin = wallet.Coins.Items.Single();
				...
				firstCoin = wallet.Coins.Items.OrderBy(x => x.Height).First();
				var secondCoin = wallet.Coins.Items.OrderBy(x => x.Height).Take(2).Last();
				var thirdCoin = wallet.Coins.Items.OrderBy(x => x.Height).Last();
				...
				Assert.NotEmpty(wallet.Coins.Items.Where(x => x.TransactionId == txid4));
				var tip = await rpc.GetBestBlockHashAsync();
				await rpc.InvalidateBlockAsync(tip); // Reorg 1
				tip = await rpc.GetBestBlockHashAsync();
				await rpc.InvalidateBlockAsync(tip); // Reorg 2
				...
				Assert.Empty(wallet.Coins.Items.Where(x => x.TransactionId == txid4));
				Assert.NotEmpty(wallet.Coins.Items.Where(x => x.TransactionId == tx4bumpRes.TransactionId));
				var rbfCoin = wallet.Coins.Items.Where(x => x.TransactionId == tx4bumpRes.TransactionId).Single();
				...
				Assert.NotEmpty(wallet.Coins.Items.Where(x => x.TransactionId == txid5));
				var mempoolCoin = wallet.Coins.Items.Where(x => x.TransactionId == txid5).Single();
		...
		public async Task SendTestsFromHiddenWalletAsync() // These tests are taken from HiddenWallet, they were tests on the testnet.
		{
			...
			try
			{
				...
				var waitCount = 0;
				while (wallet.Coins.Items.Sum(x => x.Amount) == Money.Zero)
				{
				...
				Assert.Contains(res2.InnerWalletOutputs.Single(), wallet.Coins.Items);

				#region Basic

				Script receive = wallet.GetReceiveKey("Basic").GetP2wpkhScript();
				Money amountToSend = wallet.Coins.Items.Where(x => !x.SpentOrCoinJoinInProgress).Sum(x => x.Amount) / 2;
				...
				amountToSend = wallet.Coins.Items.Where(x => !x.SpentOrCoinJoinInProgress).Sum(x => x.Amount) / 3;
				...
				Assert.Equal(wallet.Coins.Items.Where(x => !x.SpentOrCoinJoinInProgress).Sum(x => x.Amount) - res.Fee, maxBuiltTxOutput.Value);
				...
				res = await wallet.BuildTransactionAsync(password, new[] { new WalletService.Operation(receive, Money.Zero, "foo") }, 1008,
					...
					allowedInputs: wallet.Coins.Items.Where(x => !x.SpentOrCoinJoinInProgress).Select(x => new TxoRef(x.TransactionId, x.Index)).Take(1));
				...
				Assert.Contains($"{Helpers.Constants.ChangeOfSpecialLabelStart}outgoing, outgoing2{Helpers.Constants.ChangeOfSpecialLabelEnd}", wallet.Coins.Items.Where(x => x.Height == Height.MemPool).Select(x => x.Label));
				...
				Assert.Contains($"{Helpers.Constants.ChangeOfSpecialLabelStart}outgoing, outgoing2{Helpers.Constants.ChangeOfSpecialLabelEnd}", wallet.Coins.Items.Where(x => x.Height == bestHeight).Select(x => x.Label));
				Assert.Contains($"{Helpers.Constants.ChangeOfSpecialLabelStart}outgoing, outgoing2{Helpers.Constants.ChangeOfSpecialLabelEnd}", keyManager.GetKeys().Select(x => x.Label));
				...
				var allowedInputs = wallet.Coins.Items.Where(x => !x.SpentOrCoinJoinInProgress).Select(x => new TxoRef(x.TransactionId, x.Index)).Take(1);
		...
		public async Task BuildTransactionReorgsTestAsync()
		{
			...
			Assert.Empty(wallet.Coins.Items);
			...
			try
			{
				...
				Assert.Empty(wallet.Coins.Items.Where(x => x.Confirmed));
				...
				Assert.Single(wallet.Coins.Items.Where(x => !x.Confirmed));
				...
				Assert.Single(wallet.Coins.Items.Where(x => !x.Confirmed));
				Assert.Single(wallet.Coins.Items.Where(x => x.TransactionId == fundingBumpTxid.TransactionId));
				...
				Assert.Single(wallet.Coins.Items.Where(x => x.Confirmed && x.TransactionId == fundingBumpTxid.TransactionId));
			}
			finally
			{
				wallet?.Dispose();
		...
		public async Task SpendUnconfirmedTxTestAsync()
		{
			...
			try
			{
				...
				Assert.Contains(wallet.Coins.Items, x => x.TransactionId == tx1Res.Transaction.GetHash());

				// There is a coin destroyed
				Assert.Equal(1, wallet.Coins.Items.Count(x => x.SpentOrCoinJoinInProgress && x.SpenderTransactionId == tx1Res.Transaction.GetHash()));

				// There is at least one coin created from the destruction of the first coin
				Assert.Contains(wallet.Coins.Items, x => x.SpentOutputs.Any(o => o.TransactionId == tx0Id));

				var totalWallet = wallet.Coins.Items.Where(c => !c.SpentOrCoinJoinInProgress).Sum(c => c.Amount);
				...
				// There is a coin created by the latest spending transaction
				Assert.Contains(wallet.Coins.Items, x => x.TransactionId == tx2Res.Transaction.GetHash());

				// There is a coin destroyed
				Assert.Equal(1, wallet.Coins.Items.Count(x => x.SpentOrCoinJoinInProgress && x.SpenderTransactionId == tx2Res.Transaction.GetHash()));

				// There is at least one coin created from the destruction of the first coin
				Assert.Contains(wallet.Coins.Items, x => x.SpentOutputs.Any(o => o.TransactionId == tx1Res.Transaction.GetHash()));

				totalWallet = wallet.Coins.Items.Where(c => !c.SpentOrCoinJoinInProgress).Sum(c => c.Amount);
				...
				Assert.True(wallet.Coins.Items.All(x => x.Confirmed));
			}
			finally
			{
				wallet?.Dispose();
		...
		public async Task CcjTestsAsync()
		{
			...
			using (var satoshiClient = new SatoshiClient(baseUri))
			{
				...
				using (var aliceClient = await AliceClient.CreateNewAsync(network, new Key().PubKey.GetAddress(network), BlindedData, new InputProofModel[] { new InputProofModel { Input = coin.Outpoint.ToTxoRef(), Proof = key.SignMessage(ByteHelpers.ToHex(BlindedData)) } }, baseUri))
				{
					Assert.NotNull(aliceClient.BlindedOutputSignature);
					Assert.NotEqual(Guid.Empty, aliceClient.UniqueId);
					Assert.True(aliceClient.RoundId > 0);
				...
				using (var aliceClient1 = await AliceClient.CreateNewAsync(network, new Key().PubKey.GetAddress(network), blinded1.BlindedData, new InputProofModel[] { new InputProofModel { Input = input1.ToTxoRef(), Proof = key1.SignMessage(blindedOutputScriptHex1) } }, baseUri))
				using (var aliceClient2 = await AliceClient.CreateNewAsync(network, new Key().PubKey.GetAddress(network), blinded2.BlindedData, new InputProofModel[] { new InputProofModel { Input = input2.ToTxoRef(), Proof = key2.SignMessage(blindedOutputScriptHex2) } }, baseUri))
				{
		...
		public async Task CoinJoinMultipleRoundTestsAsync()
		{
			...
			try
			{
				...
				waitCount = 0;
				while (wallet2.Coins.Items.Sum(x => x.Amount) == Money.Zero)
				{
					await Task.Delay(1000);
					waitCount++;
					if (waitCount >= 21)
				...
				Assert.True(1 == (await chaumianClient.QueueCoinsToMixAsync(password, wallet.Coins.Items.ToArray())).Count());
				Assert.True(3 == (await chaumianClient2.QueueCoinsToMixAsync(password, wallet2.Coins.Items.ToArray())).Count());
				...
				var times = 0;
				while (wallet.Coins.Items.Where(x => x.Label == "ZeroLink Change" && x.Unspent).SingleOrDefault() is null)
				{
					await Task.Delay(1000);
					times++;
					if (times >= 21) throw new TimeoutException("Wallet spends were not recognized.");
				}
				SmartCoin[] unspentChanges = wallet.Coins.Items.Where(x => x.Label == "ZeroLink Change" && x.Unspent).ToArray();
				await wallet.ChaumianClient.DequeueCoinsFromMixAsync(unspentChanges);

				Assert.Equal(3, wallet.Coins.Items.Count(x => x.Label == "ZeroLink Mixed Coin" && !x.SpentOrCoinJoinInProgress));
				Assert.Equal(3, wallet2.Coins.Items.Count(x => x.Label == "ZeroLink Mixed Coin" && !x.SpentOrCoinJoinInProgress));
				Assert.Equal(0, wallet.Coins.Items.Count(x => x.Label == "ZeroLink Mixed Coin" && !x.Unspent));
				Assert.Equal(0, wallet2.Coins.Items.Count(x => x.Label == "ZeroLink Mixed Coin" && !x.Unspent));
				Assert.Equal(2, wallet.Coins.Items.Count(x => x.Label == "ZeroLink Change" && !x.Unspent));
				Assert.Equal(0, wallet2.Coins.Items.Count(x => x.Label == "ZeroLink Change"));
				Assert.Equal(0, wallet.Coins.Items.Count(x => x.Label == "ZeroLink Change" && x.Unspent));
				Assert.Equal(0, wallet.Coins.Items.Count(x => x.Label == "ZeroLink Dequeued Change" && !x.Unspent));
				Assert.Equal(1, wallet.Coins.Items.Count(x => x.Label == "ZeroLink Dequeued Change" && !x.SpentOrCoinJoinInProgress));
			}
			finally
			{
				wallet.NewFilterProcessed -= Wallet_NewFilterProcessed;
