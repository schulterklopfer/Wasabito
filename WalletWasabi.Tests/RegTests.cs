...
namespace WalletWasabi.Tests
{
	...
	public class RegTests : IClassFixture<SharedFixture>
	{
		...
		public async Task CcjTestsAsync()
		{
			...
			var roundConfig = new CcjRoundConfig(denomination, 2, coordinatorFeePercent, anonymitySet, 100, connectionConfirmationTimeout, 50, 50, 2, 24, 2);
		...
		public async Task BanningTestsAsync()
		{
			...
			var roundConfig = new CcjRoundConfig(denomination, 140, coordinatorFeePercent, anonymitySet, 240, connectionConfirmationTimeout, 1, 1, 1, 24, 2);
			coordinator.UpdateRoundConfig(roundConfig);
			coordinator.AbortAllRoundsInInputRegistration(nameof(RegTests), "");
		...
		public async Task Ccj100ParticipantsTestsAsync()
		{
			...
			int connectionConfirmationTimeout = 120;
			var roundConfig = new CcjRoundConfig(denomination, 144, coordinatorFeePercent, anonymitySet, 240, connectionConfirmationTimeout, 50, 50, 1, 24, 2);
			coordinator.UpdateRoundConfig(roundConfig);
			coordinator.AbortAllRoundsInInputRegistration(nameof(RegTests), "");
		...
		public async Task CcjClientTestsAsync()
		{
			...
			var roundConfig = new CcjRoundConfig(denomination, 140, coordinatorFeePercent, anonymitySet, 240, connectionConfirmationTimeout, 50, 50, 1, 24, 2);
			coordinator.UpdateRoundConfig(roundConfig);
			coordinator.AbortAllRoundsInInputRegistration(nameof(RegTests), "");
			await rpc.GenerateAsync(3); // So to make sure we have enough money.
			var keyManager = KeyManager.CreateNew(out _, password);
			...
			try
			{
				...
				roundConfig = new CcjRoundConfig(denomination, 140, coordinatorFeePercent, anonymitySet, 240, connectionConfirmationTimeout, 50, 50, 1, 24, 2);
				coordinator.UpdateRoundConfig(roundConfig);
				coordinator.AbortAllRoundsInInputRegistration(nameof(RegTests), "");
