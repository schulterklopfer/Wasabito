...
namespace WalletWasabi.Tests
{
	class TesteableRpcService : JsonRpcService
	{
		...
		public JsonRpcResponse Echo(string text) => Result(text);

		[JsonRpcMethod("substract", "Substracts two numbers.")]
		public JsonRpcResponse Substract(int minuend, int subtrahend) => Result(minuend - subtrahend);
		...
		public async Task<JsonRpcResponse> FormatHardDriveAsync(string unit, CancellationTokenSource cts)
		{ 
			return await Task.FromResult<JsonRpcResponse>(Result((object)null));
		}
	}
